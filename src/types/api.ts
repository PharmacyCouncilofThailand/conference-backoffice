...
export type StudentEligibilityStatus =
  | "pending"
  | "approved"
  | "rejected"
  | "cancelled";
...
// Team registrations are an isolated domain and do not reuse ticket registrations.
export type TeamRegistrationStatus =
  ...
  | "paid"
  | "expired";
export type TeamPaymentAttemptStatus =
  | "creating"
  | "pending"
  | "paid"
  | "failed"
...
export interface TeamRegistrationListItem {
  ...
  category: {
    code: string;
    name: string;
    educationLevel: "higher_education" | "upper_secondary";
  };
  leader: { name: string; email: string };
  memberCount: number;
  pricingRound: string | null;
  amount: string | null;
  currency: string | null;
  registrationStatus: TeamRegistrationStatus;
  paymentStatus: TeamPaymentAttemptStatus | null;
  paidAt: string | null;
  createdAt: string;
}
...
export interface TeamRegistrationDetail {
  registration: Record<string, unknown> & {
    ...
    pricingRoundNameSnapshot: string | null;
  };
  ...
  paymentAttempts: Array<
    Record<string, unknown> & {
      ...
      amount: string;
      currency: string;
      status: TeamPaymentAttemptStatus;
      createdAt: string;
      paidAt: string | null;
    }
  >;
...
export interface TeamRegistrationCategoryConfig {
  id?: number;
  code: string;
  displayName: string;
  educationLevel: "higher_education" | "upper_secondary";
  pharmacyRule: "required" | "forbidden";
  isActive: boolean;
  displayOrder: number;
}
export interface TeamRegistrationRoundConfig {
  id?: number;
  code: string;
  displayName: string;
  startsAt: string;
...
export interface TeamRegistrationPriceConfig {
  id?: number;
...
export interface TeamRegistrationConfig {
  ...
  paymentAttemptTtlMinutes: number;
  paymentProfileCode: string;
  eventWebsiteOrigin: string;
  paymentResultUrl: string;
  categories: TeamRegistrationCategoryConfig[];
  pricingRounds: TeamRegistrationRoundConfig[];
  prices: TeamRegistrationPriceConfig[];
}
